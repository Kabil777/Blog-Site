import {
  ...
  function Form({ open, closeReq }) {
	...
	const handleTabChange = (_, newValue = null) => {
	  ...
	  if (titleValue.length > 35 || titleValue.length < 30) {
		setOpenSnackbar(true);
		setSnackbarMessage("Invalid input, Minimum 30 letters and Maximum 35 letters");
	...
	return (
	  <>
		...
		>
		  <DialogTitle fontSize="1.5em" fontWeight="600" fontFamily="Lato" sx={{ mt: "10px" }}>
			<IconButton sx={{ border: "2px solid #e4e4e7" }} disableFocusRipple disableTouchRipple>
			  <BsPinAngleFill />
			</IconButton>{" "}
			Article Headers
		  </DialogTitle>
		  <Tabs
			...
			sx={{ ml: "25px", mb: "15px" }}
		  >
			<Tab label="Title" wrapped sx={{ fontSize: "0.9rem", fontWeight: "600", fontFamily: "Lato" }} />
			<Tab label="Tags" sx={{ fontSize: "0.9rem", fontWeight: "600", fontFamily: "Lato" }} />
		  </Tabs>
		  ...
		  <div style={{ width: "100%", height: "345px" }} hidden={activeTab !== 1}>
			<DialogContent>
			  ...
			  >
				{chips.map((chip) => {
				  const isClicked = clickedChips.some((clicked) => clicked.name === chip.name);
				  ...
				  return (
					<Box key={chip.key} sx={{ m: "15px 5px", height: "25px" }}>
					  <Chip
						...
						onDelete={isClicked ? () => deleteAction(chip.name) : undefined}
